...
class CategoryDataset(Dataset):
    ...
    def __init__(self, root_dir="../data/images2/",  # images2对应的原始文件目录在/home/ices/xwj/graduation project/dataset/img/img_types/total/
                 ...
                 data_dir="../data", transform=None, use_mean_img=True, neg_samples=True, generator_type="upper"):
    ...
    def __getitem__(self, index):
        ...
        while choice[0] == set_id:
            choice = random.choice(self.data)
        img_path = os.path.join(self.root_dir, str(choice[1][category[change_id]]['index'])+'.jpg')
        # names.append(torch.LongTensor(self.str_to_idx(choice[1][category[change_id]]['name'])))
        labels.append('{}_{}_{}'.format(choice[0], category[change_id], choice[1][category[change_id]]['index']))
        img = Image.open(img_path).convert('RGB')
        img = self.transform(img)
        imgs.append(img)
        input_images = torch.stack(imgs)  # 沿着第0维度拼接图片 [N,C,H,W], N = len(imgs), [5, 3, 224, 224]
        # names = torch.stack(names)
        return input_images, names, set_id, labels, change_id
    ...
    def str_to_idx(self, name):
        name_len = len(self.vocabulary)  # 生成一个0-1向量  1244+1 = 1245维
        desc = [0] * name_len
...
def collate_fn(data):
    ...
    return (lengths, images, names, set_id, labels, change_ids)

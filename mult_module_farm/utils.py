...
class BestSaver(object):
    ...
    def __init__(self, comment=None):
        ...
        auc_save_path = "data_mix_model_{}".format(exe_fname.split(".")[0])
        ...
        diff_acc_save_path = "data_mix_model_diff_acc_{}".format(exe_fname.split(".")[0])
        if comment is not None and str(comment):
        ...
        pos_neg_save_path = "data_mix_model_pos_neg_acc_{}".format(exe_fname.split(".")[0])
        if comment is not None and str(comment):
        ...
        mix_save_path = "data_mix_mix_acc_{}".format(exe_fname.split(".")[0])
        if comment is not None and str(comment):
...
def prepare_dataloaders(root_dir="../data/images2/", data_dir="../data/", batch_size=16, generator_type="upper",
                        img_size=128, use_mean_img=True, neg_samples=True,

...
# Compatibility AUC test # F_AUC: 0.6587, B_AUC: 0.6656, ALL_AUC: 0.6651
def compat_test(test_dataset):
    ...
    print('test:{} Compatibility: F_AUC: {:.4f}, B_AUC: {:.4f}, ALL_AUC: {:.4f}'.format(epoch + 1, f_auc, b_auc, all_auc))
    return all_auc
...
def fitb_test(test_dataset):
    ...
    all_acc = sum(is_correct_all) / len(is_correct_all)
    print("test:{} FITB : F_ACC: {:.4f}, B_ACC: {:.4f}, All_ACC: {:.4f}".format(
        epoch + 1,

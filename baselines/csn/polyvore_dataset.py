...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        anchor_img_path = os.path.join(self.root_dir, str(parts[anchor_part]['index']) + '.jpg')
        pos_img_path = os.path.join(self.root_dir, str(parts[pos_part]['index']) + '.jpg')

        neg_choice = self.data[index]
        while (pos_part not in neg_choice[1]) or (neg_choice[0] == set_id):
            neg_choice = random.choice(self.data)
        neg_img_path = os.path.join(self.root_dir, str(neg_choice[1][pos_part]['index']) + '.jpg')
